...
public class UserLaceController {
    ...
    private void setupColumns() {
                    ...
                    if (product != null) {
                        ...
                        if (availableStock > 0) {
                            quantitySpinner.setValueFactory(
                                    new SpinnerValueFactory.IntegerSpinnerValueFactory(1, availableStock, 1)
                            );
        ...
        productimagecol.setCellFactory(param -> new TableCell<>() {
    ...
    private void loadLaceProducts() {
        productList.clear();
        String query = "SELECT * FROM product WHERE category_id = 'C005 - LACE'";
        try (Connection conn = DatabaseHandler.getDBConnection();
