...
public class UserShirtController {
    ...
    public void initialize() {
        if (!ProductTransfer.getShirtProductList().isEmpty()) {
            selectedShirt = ProductTransfer.getShirtProductList().get(0);
    ...
    private void loadProductDetails(Product product) {
        ...
        int availableQuantity = getAvailableStockFromDB(product.getProductId());

        if (availableQuantity > 0) {
            SpinnerValueFactory<Integer> factory = new SpinnerValueFactory.IntegerSpinnerValueFactory(1, availableQuantity, 1);
            quantitySpinner.setValueFactory(factory);
            quantitySpinner.setDisable(false);

            quantitySpinner.valueProperty().addListener((obs, oldVal, newVal) -> {
                if (newVal > availableQuantity) {
                    quantitySpinner.getValueFactory().setValue(Math.min(oldVal, availableQuantity));
                    showAlert("Stock Limit", "Only " + availableQuantity + " item(s) available in stock.");
                }
            });
        } else {
    ...
    private void handleAddToCartSingleView() {
        ...
        int availableQuantity = getAvailableStockFromDB(selectedShirt.getProductId());
            ...
            protected void updateItem(Double price, boolean empty) {
                ...
                if (empty || price == null) {
                    ...
                    if (product != null) {
                        ...
                        if (availableStock > 0) {
                            valueFactory = new SpinnerValueFactory.IntegerSpinnerValueFactory(1, availableStock, 1);
                            quantitySpinner.setDisable(false);
                        } else {
